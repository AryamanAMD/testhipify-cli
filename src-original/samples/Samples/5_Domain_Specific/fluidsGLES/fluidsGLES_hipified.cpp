...
GLuint m_texture = 0;
struct hipGraphicsResource *cuda_vbo_resource; // handles OpenGLES-CUDA exchange
...
void keyboard(unsigned char key, int x, int y, int argc, char **argv)
{
    switch (key)
    {
        ...
        case 'r':
            ...
            hipMemcpy(dvfield, hvfield, sizeof(cData) * DS, hipMemcpyHostToDevice);
...
void cleanup(void)
{
    ...
    free(hvfield);
    free(particles);
    checkCudaErrors(hipFree(dvfield));
    checkCudaErrors(hipFree(vxfield));
    checkCudaErrors(hipFree(vyfield));
...
int main(int argc, char **argv)
{
    ...
    checkCudaErrors(hipMallocPitch((void **)&dvfield, &tPitch, sizeof(cData)*DIM, DIM));

    checkCudaErrors(hipMemcpy(dvfield, hvfield, sizeof(cData) * DS, hipMemcpyHostToDevice));
    // Temporary complex velocity field data
    checkCudaErrors(hipMalloc((void **)&vxfield, sizeof(cData) * PDS));
    checkCudaErrors(hipMalloc((void **)&vyfield, sizeof(cData) * PDS));
